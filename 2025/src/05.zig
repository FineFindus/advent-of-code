...
fn parse_fresh_ingredients(input: []const u8, allocator: std.mem.Allocator) !std.ArrayList(Interval) {
    var fresh_ingredients_it = std.mem.splitSequence(u8, input, "\n");
...
pub fn part1(input: []const u8, allocator: std.mem.Allocator) !usize {
    ...
    var fresh_ingredients = try parse_fresh_ingredients(it.next().?, allocator);
    defer fresh_ingredients.deinit(allocator);

    std.sort.pdq(Interval, fresh_ingredients.items, {}, Interval.sortFn);

    var sum: usize = 0;
    var available_ingredients = std.mem.splitSequence(u8, it.next().?, "\n");
